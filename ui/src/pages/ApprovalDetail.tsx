...
export function ApprovalDetail() {
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "请求修订失败"),
  });
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">决定备注: {approval.decisionNote}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">关联的任务</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              请在 <Link to="/costs" className="underline underline-offset-2">/costs</Link> 的预算控制中解决此预算停止。
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("删除此被拒绝的智能体？此操作无法撤销。")) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? "提交中..." : "发表评论"}
          </Button>
        </div>
      </div>
    </div>
