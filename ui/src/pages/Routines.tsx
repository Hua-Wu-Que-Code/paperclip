...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "如果已有活跃运行，则仅保留一个后续排队运行。",
  always_enqueue: "将每次触发都加入队列，即使例程已在运行。",
  skip_if_active: "当运行仍处于活跃状态时，丢弃新的触发。",
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: "忽略调度器或例程暂停期间错过的窗口。",
  enqueue_missed_with_cap: "恢复后以有上限的批次追赶错过的调度窗口。",
};
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-2">
            ...
            <span>{routine.assigneeAgentId ? (agent?.name ?? "未知智能体") : "无默认智能体"}</span>
          </span>
          <span>
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        <div className="flex items-center gap-3">
          ...
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? "已归档" : isDraft ? "草稿" : enabled ? "开" : "关"}
          </span>
        </div>
...
export function Routines() {
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: "更新例程失败",
        body: mutationError instanceof Error ? mutationError.message : "无法更新例程。",
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: "例程运行失败",
        body: mutationError instanceof Error ? mutationError.message : "无法启动例程运行。",
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message="选择一个公司以查看例程。" />;
  }
        ...
        >
              ...
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">将策略控制置于工作定义之下。</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">补追策略</p>
                      <Select
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              创建后，Paperclip 会直接跳转到触发器设置。草稿例程在添加默认智能体之前会保持暂停状态。
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? "创建中..." : "创建例程"}
              </Button>
        ...
        <div>
          {(routines ?? []).length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message="暂无例程。使用创建例程来定义第一个循环工作流。"
              />
            </div>
          ) : (
