...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message="选择一个文件以预览其内容。" />
    );
  }
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    "上传直接从 Paperclip 导出的 .zip 文件。通过 Finder、资源管理器或其他 zip 工具重新压缩的存档可能无法正确导入。";
  ...
  const previewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: "预览失败",
        body: err instanceof Error ? err.message : "预览导入失败。",
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: "导入完成",
        body: `${result.company.name}：已处理 ${result.agents.length} 个智能体。`,
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: "导入失败",
        body: err instanceof Error ? err.message : "应用导入失败。",
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: "读取包失败",
        body: err instanceof Error ? err.message : "读取文件夹失败。",
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message="选择一个公司进行导入。" />;
  }
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: "GitHub 仓库" },
              { key: "local", icon: Upload, label: "本地 zip 包" },
            ] as const
          ...
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {localPackage.name}，共{" "}
                  {Object.keys(localPackage.files).length} 个文件
                </span>
              )}
            </div>
            {!localPackage && (
        ...
        >
          ...
          >
            ...
            <option value="replace">替换冲突</option>
          </select>
        </Field>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {selectedCount} / {totalFiles} 个文件已选择
              </span>
              {conflicts.length > 0 && (
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">打包文件</h2>
              </div>
