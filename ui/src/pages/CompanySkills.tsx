...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    case "skills_sh":
      return { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: "skills.sh 管理" };
    case "github":
      return isSkillsShManaged
        ? { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: "skills.sh 管理" }
        : { icon: Github, label: sourceLabel ?? "GitHub", managedLabel: "GitHub 管理" };
    case "url":
      return { icon: Link2, label: sourceLabel ?? "URL", managedLabel: "URL 管理" };
    case "local":
      return { icon: Folder, label: sourceLabel ?? "文件夹", managedLabel: "文件夹管理" };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Paperclip", managedLabel: "Paperclip 管理" };
    default:
      return { icon: Boxes, label: sourceLabel ?? "目录", managedLabel: "目录管理" };
  }
}
...
function formatProjectScanSummary(result: CompanySkillProjectScanResult) {
  const parts = [
    `${result.discovered} 已发现`,
    `${result.imported.length} 已导入`,
    `${result.updated.length} 已更新`,
  ];
  if (result.conflicts.length > 0) parts.push(`${result.conflicts.length} 个冲突`);
  if (result.skipped.length > 0) parts.push(`${result.skipped.length} 个跳过`);
  return `${parts.join("，")}，共 ${result.scannedWorkspaces} 个工作区。`;
}
    ...
    <div className="min-w-0">
      <div className="border-b border-border px-5 py-4">
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">版本锁定</span>
          ...
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            ...
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">无关联代理</span>
            ) : (
      ...
      <div className="min-h-[560px] px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">选择一个文件来查看。</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
  ...
  const importSkill = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "技能导入失败",
        body: error instanceof Error ? error.message : "无法导入技能来源。",
      });
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "技能创建失败",
        body: error instanceof Error ? error.message : "无法创建技能。",
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onError: (error) => {
      ...
      pushToast({
        tone: "error",
        title: "项目技能扫描失败",
        body: error instanceof Error ? error.message : "无法扫描项目工作区。",
      });
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "保存失败",
        body: error instanceof Error ? error.message : "无法保存技能文件。",
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "更新失败",
        body: error instanceof Error ? error.message : "无法安装技能更新。",
      });
    },
  });
  ...
  const deleteSkill = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: "删除失败",
        body: error instanceof Error ? error.message : "无法移除技能。",
      });
    },
  });

  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message="请选择一个公司来管理技能。" />;
  }
          ...
          <div className="space-y-3 text-sm">
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-muted-foreground">
                  查找包含 `SKILL.md` 的仓库，然后在此粘贴仓库 URL。
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
