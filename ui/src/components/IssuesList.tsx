    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? "关闭父子嵌套" : "开启父子嵌套"}
            >
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          最多显示 {ISSUE_SEARCH_RESULT_LIMIT} 条匹配结果。请优化搜索条件以缩小范围。
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          部分看板列最多显示 {ISSUE_BOARD_COLUMN_RESULT_LIMIT} 条任务。请优化筛选条件或搜索以查看更多。
        </p>
      )}
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message="没有匹配当前筛选条件或搜索的任务。"
      ...
      {viewState.viewMode === "board" ? (
        ...
        <>
          ...
          {remainingIssueRowCount > 0 && (
            <p className="text-xs text-muted-foreground">
              正在渲染 {Math.min(renderedIssueRowLimit, filtered.length)} / {filtered.length} 条任务
            </p>
          )}
        </>
      )}
