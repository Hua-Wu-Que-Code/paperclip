  ...
  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? "归档此项目以将其从侧边栏和项目选择器中隐藏。"
          : "取消归档此项目以将其恢复到侧边栏和项目选择器中。"}
      </p>
      {archivePending ? (
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{action}项目</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{action}项目</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error("请选择一个公司以创建密钥");
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError("仓库必须使用有效的 GitHub 或 GitHub Enterprise 仓库 URL。");
      return;
    }
    setWorkspaceError(null);
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              应用于此项目中所有问题的运行。项目值在键冲突时覆盖代理环境变量。
            </p>
          </div>
        </PropertyRow>
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <Tooltip>
              ...
              <TooltipContent side="top">
                仓库标识真实来源。本地文件夹是代理写入代码的默认位置。
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">仓库</div>
              {codebase.repoUrl ? (
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">本地文件夹</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">Paperclip 管理的文件夹。</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? "更改本地文件夹" : "设置本地文件夹"}
                  </Button>
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                此项目上存在其他旧版工作区记录。Paperclip 正在使用主工作区作为代码库视图。
              </div>
            )}
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          主机管理的实现方式：<span className="text-foreground">Git worktree</span>
                        </div>
                        {environmentsEnabled ? (
                          <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          配置命令在代理执行之前于派生的 worktree 中运行。清理命令存储在此处，用于将来的清理流程。
                        </p>
                      </div>
                    ) : null}
                  </div>
