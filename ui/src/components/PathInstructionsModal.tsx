...
const instructions: Record<Platform, { steps: string[]; tip?: string }> = {
  mac: {
    steps: [
      "打开 Finder 并导航到目标文件夹。",
      "右键点击（或按住 Control 键点击）该文件夹。",
      "按住 Option (⌥) 键——\"拷贝\"会变为\"拷贝为路径名\"。",
      "点击\"拷贝为路径名\"，然后粘贴到此处。",
    ],
    tip: "你也可以打开终端，输入 cd，然后将文件夹拖入终端窗口并按回车。然后输入 pwd 即可查看完整路径。",
  },
  windows: {
    steps: [
      "打开文件资源管理器并导航到目标文件夹。",
    ...
    tip: "或者，按住 Shift 键并右键点击文件夹，然后选择\"复制为路径\"。",
  },
  linux: {
    steps: [
      "打开终端，使用 cd 导航到目标目录。",
    ...
    tip: "在大多数文件管理器中，按 Ctrl+L 可以在地址栏中显示完整路径。",
  },
};
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            将绝对路径（例如{" "}
            <code className="text-xs bg-muted px-1 py-0.5 rounded">/Users/你/项目</code>
            ）粘贴到输入框中。
          </DialogDescription>
        </DialogHeader>
